...
def create_init_dict(filename):
    ...
    for i in range(len(data)):
        data_dict[i] = (data.iloc[i]['lng'], data.iloc[i]['lat'])
    print(data_dict)
    return data_dict
...
def shuffle(my_list):  # 起点和终点不能打乱
    ...
    shuffle_list = temp_list
    # print(shuffle_list)
    return shuffle_list


def product_len_probability(my_list, d_matrix, size, p_num):
    # print('----------------------------product my list----------------')
    # # print(my_list[0])
    # print('----------------------------product my list end----------------')
    ...
    # print( my_list[gen_best_length_index])
...
if __name__ == "__main__":
    ...
    excel = 'd:\\data\\项目经纬度表.xlsx'
    data = create_init_dict('d:\\data\\项目经纬度表.xlsx')
    # print (data)
    start = time.time()
    # size=10
    p_num = 100  # 种群个体数量
    ...
    coordinate_dict = create_init_dict('d:\\data\\项目经纬度表.xlsx')
