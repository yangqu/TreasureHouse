...
def parellel_compare(row1, row2, p):
    print(row1)
    for i in range(row1.index.values.__len__()):
        row_data1 = row1.iloc[i]
        max_simi = 0
        address1 = row_data1
        address2 = ''
        for j in range(row2.index.values.__len__()):
            row_data2 = row2.iloc[j]
...
def main():
    ...
    step = int(address1.index.values.__len__()/number)
    print(address1.index.values.__len__())
    for i in range(number):
        print(i * step)
        q = None
        if i == number - 1:
            q = address1.iloc[i * step:]
        else:
            q = address1.iloc[i * step:(i + 1) * step]
        t1 = threading.Thread(target=parellel_compare, args=(q, address2, i))
        threads.append(t1)
        t1.start()
